...
async def process(request: dict):
    ...
    await send_log(f"Received data from KIO7")
    ...
    try:
        ...
        await send_log(f"Compliance check passed for patient {message.get('payload', {}).get('patient_id', 'unknown')}")
    except Exception as e:
    ...
    await send_log("Returning final report to UI")
